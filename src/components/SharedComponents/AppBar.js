...
function AppBar({scrolldown}){
    const topClass = "fixed top-0 z-9999 bg-darker-1 flex flex-col md:flex-row w-full md:h-24 items-center justify-center gap-4 px-8 py-6 md:py-0 duration-500 drop-shadow-xl";
    ...
    return(
        <div className={scrolldown ? topClassScroll : topClass}>
            ...
            <ul className="inline flex gap-6 items-center justify-center h-full">
                {menus.map((menu) => {
                    return (
                        <li id={menu.id}>
                            <a href={menu.url} className="tracking-wider text-md font-thin text-light-1 inline hover:text-yellow-1 hover:cursor-pointer font-roboto">
                                {menu.name}
                            </a>
                        </li>
                    );
